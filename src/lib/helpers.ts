...
export function isDateWithinRange(baseDate: Date, compareDate: Date, interval: number): boolean {
...
export function isNumberWithinRange(baseValue: string, compareValue: string): boolean {
...
export function truncateEthereumAddress(address: string): string {
  ...
  if (address.length <= 2 + prefixLength + suffixLength) {
      return address;  // If the address is too short, return it unmodified
  }
